# Bevis-matrise — AHO-saka

Samla oversikt over alle påstandar og funn på tvers av researchrunde 1–3, med
status, styrke og primærkjelde. **Nøytralt og bevisbasert** — dette er ein
arbeidsmatrise, ikkje ein publiseringsklar konklusjon. Sjå rundelogg:
[`research/`](research/). Status: 🟢 stadfesta · 🟡 delvis/nyansert · 🔴 uverifisert/
treng innsyn · ⛔ avkrefta/overdrive. Styrke: Sterk / Medium / Svak.

## A — AHO-spesifikke, etterprøvbare påstandar (frå brevet til Killi)

| ID | Påstand | Status | Styrke | Primærkjelde / korleis vidare |
| --- | --- | --- | --- | --- |
| A1 | Sept 2025: designstudentar til «C4» (politiet Amsterdam); «live demo» av Palantir | 🔴 | Svak | «C4»/demo ikkje eksternt stadfesta (nær: RTIC). Innsyn til AHO (`maler/utkast/A1`) |
| A2 | GK4-oppgåve bad om «legge til en bærekraftshandling i designet» | 🔴 | — | Innsyn til AHO (`A2`) |
| A3 | Rektor avgangstale 12.06.2026: «menneskelig» teknologi; berekraft «ikke et tillegg» | 🔴 | — | Innsyn/opptak (`A3`) |
| A4 | Rektor: kritisk tenking «kjennetegner utdanningene våre» | 🟢 | Medium | Rektor Irene Alma Lønne, arkitektur.no 23.04.2026 |
| A5 | «et selvfølgelig valg for … sosialt og klimamessig bærekraftig framtid» | 🟢 | Medium | Rektor, arkitektur.no 26.04.2024 |
| A6 | Studieplan-sitatet om etikk | 🟡 | Medium | Sitatet er **NKR-deskriptoren** (stadfesta NOKUT), ikkje AHOs studieplan; AHO har svakare «reflektere over og posisjonere» |
| A7 | Vurdering skal vere tilpassa læringsutbytet (lovpålagt) | 🟢 | Sterk | UH-forskrifta **§ 1-11 c/e** (FOR-2024-06-28-1392), erstatta oppheva studietilsynsforskrift § 2-2 |
| A8 | Etikk-utbytet vert ikkje prøvd/vurdert | 🟡 | Medium | Ingen etikk-emne/vurderingsform i studieplan; emnenivå → innsyn (`A8`). Støtta av NOKUT § 4-1(3) 2018 |
| A9 | AHO sjølvakkrediterer; ekstern kontroll = kvalitetssystem | 🟢 | Sterk | UH-lova § 3-4/§ 3-5; NOKUT (vitskapleg høgskule) |
| A10 | Inga designspesifikk etisk «nei»-instans | 🟢 | Sterk | Forskingsetisk organ finst, men **Riksrevisjonen (2022)** namngir AHO for manglande oppfølging av forskingsetiske retningsliner; **inga due-diligence av eksterne partnarar i nokon styresak** (C11); ingen reservasjonsrett (benchmark) |
| A11 | «AHO éin av berre 2 av 78» utan etisk eksternprosedyre | ⛔→🟢 | Medium | «2 av 78» **falsk** (64 koda); men **omformulert held**: 7 av 11 norske har eksternprosedyre AHO manglar |
| A12 | Kronikkutkast: dusinvis studentunderskrifter + 7 faglærarar | 🟡 | — | Intern liste; krev stadfesting |

## B — Eksterne kontekstpåstandar (Palantir / geopolitikk)

| ID | Påstand | Status | Styrke | Primærkjelde / åtvaring |
| --- | --- | --- | --- | --- |
| B1 | Raffinaderij/Palantir kjøpt av NL-politi utan anbod 2018, avdekt via rettssak | 🟢 | Sterk | NL-regjering (Kamervragen 2025–26 nr. 93); Woo-dom ECLI:NL:RBAMS:2025:4478 |
| B2 | NL «vedgjekk masseovervaking» | ⛔ | — | Overdrive: staten stadfesta bruk + løynd innkjøp, men **avviste** «massasurveillance» |
| B3a | Palantir–Israel-kontrakt (jan 2024) | 🟢 | Sterk | Stadfesta (selskapet); men konkret Gaza-måljakt-bruk **avvist av Palantir**, ikkje uavh. stadfesta |
| B3b | Palantir–ICE «ImmigrationOS» (2025) | 🟢 | Sterk | Axios, kongressbrev; skil «verktøy» frå «namngjevne razziaar» |
| B4 | Minab-skuleåtak 28.02.2026 | 🟢 | Sterk (hending) | Reuters/AP/BBC Verify/OHCHR/Amnesty/HRW. Dødstal: bruk **~156, fleirtal barn** (ikkje 160/150/148) |
| — | Palantir↔Minab-kopling | 🔴 | — | **Udokumentert — ikkje gjer denne koplinga** |
| B5/B6 | Forbrukerrådet-oppropet feb 2026; ingen designinstitusjon signerte | 🟢 | Sterk | forbrukerradet.no; «Breaking Free…»; éin designar (C.G. Falch) |

## C — Institusjonelle styrings-/historiske funn (runde 3, arkivhausting)

| ID | Funn | Status | Styrke | Primærkjelde |
| --- | --- | --- | --- | --- |
| C1 | Riksrevisjonen: AHO følgde ikkje opp forskingsetiske retningsliner | 🟢 | **Sterk** | Riksrevisjonen Dok 1 (2021–2022), 22.09.2022, s. 28/32 |
| C2 | Berkano AS (2004): KD trekte AHOs selskapsfullmakt | 🟢 | Sterk | Riksrevisjonen Dok 1 (2005–2006) |
| C3 | Rekneskapsmerknad 2009; særskild KD-oppfølging | 🟢 | Sterk | Riksrevisjonen Dok 1 (2010–2011) |
| C4 | NOKUT § 4-1(3)-mangel 2018 (internkontroll/evaluering studietilbod), retta 2019 | 🟢 | Sterk | NOKUT saksnr 17/02748 |
| C5 | Utviklingsavtale KD 2023–2026 forpliktar AHO til berekraft gjennomgåande | 🟢 | Sterk | regjeringen.no; TB2023–2025 |
| C6 | Etikk/berekraft i liten grad operasjonalisert i styrevedtak | 🟡 | Medium | Ingen «bærekraft»-vedtak 2018–20; ingen eige etikk-vedtak 2024–26 |
| C7 | Etikk-/berekraft-retorikk er programmarknadsføring, ikkje institusjonell | 🟢 | Medium | Wayback: «Om AHO» 2011–25 utan slikt språk; fråvær 2014 → marknadsføring 2019→ |
| C8 | Design-studieplassar/-årsverk bygde ned (2024–27) | 🟢 | Medium | Styresak 47/24, 49/24, 47/26. **Økonomisk/KD-driven**: AHO søkte tvert imot **+30 designplassar** i 2022 (sak 80/22) og 2023 (sak 58/23); KD trekte korona-plassane → −30–35 mill./11–13% frå 2026; kutt 3 årsverk (tjeneste-/systemorientert design). **Ikkje eit AHO-verdival** |
| C9 | KD-oppfølging av NOKUT-kontroll med samfunnssikkerhet/beredskap ved AHO | 🟡 | — | eInnsyn saksnr 2020/4663 (2021) — innsyn |
| C10 | AHO ikkje journaleining i eInnsyn (intern journal berre via direkte innsyn) | 🟢 | Sterk | eInnsyn API (metodefunn) |
| C11 | **Inga aktsemdsvurdering (due diligence) av eksterne partnarar** i styredok. | 🟢 | Sterk | Fulltekst 6 styre-saksframlegg: berre ambisjon om meir ekstern finansiering, ingen etisk screening-prosedyre → stør A10/A11 |
| C12 | Utviklingsavtale 2027–2030 mindre eksplisitt på berekraft-som-kjerne | 🟡 | Medium | Sak 43/26: tyngdepunkt portefølje/finansiering/partnarskap (vs. 2023–26 «ruste studentene til bærekraftig samfunn») |
| C13 | Gaza-utsegn (juni 2024) + studentkrav om akademisk boikott/divest | 🟢 | Medium | Sak 37/24; SAHO-vedlegg (HP/Dell, Bezalel) — kontekst for overvakingsteknologi-vinkelen |

## Samla bilete (nøytralt)

Det best dokumenterte, publiseringseigna mønsteret: **lovnad/retorikk > operativ
forankring**. AHO har (a) ei **KD-forplikting** på berekraft gjennomgåande
(utviklingsavtale), (b) **marknadsført** etikk/berekraft sterkt sidan 2019, men
(c) etikk-utbytet er relativt nytt og lite synleg vurdert, (d) **Riksrevisjonen**
har namngitt AHO for manglande oppfølging av forskingsetikk, og (e) AHO manglar —
til skilnad frå fleire norske peerar — ein etisk prosedyre for eksterne samarbeid.

**Treng innsyn for å lukke:** A1/A2/A3/A8 (AHO direkte), NOKUT-saksmappe 17/02748,
TB 2011–2016 + første utviklingsavtale (KD), AHOs revisjonsbrev 2010–2024
(Riksrevisjonen), styreprotokollar 2011–2017. Malar: [`../../maler/utkast/`](../../maler/utkast/).

**Presseetikk:** gi AHO og namngjevne personar samtidig imøtegåing før publisering.
Skil sterke (C1–C5, B1) frå svake/uverifiserte (A1, B-koplingar) funn.
